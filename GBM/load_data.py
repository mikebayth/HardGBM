...
def load_by_argv(argv_str):
    ...
    if (argv_str == 'blood'):
        ...
        X, y = load_hospital()
    elif (argv_str=='pumpkin'):
        X, y = load_pumpkin()
    elif (argv_str=='auction'):
        X, y = load_auction()
    else:
...
########### 2022-05-18 ##############
# https://archive-beta.ics.uci.edu/ml/datasets/auction+verification
def load_auction():
    global data_path
    data = read_csv(data_path+'auction.csv')
    X = data.drop(['verification.result', 'verification.time'],axis=1)
    y = data.iloc[:,7]
    le = LabelEncoder()
...
def load_pumpkin():
    global data_path
    data = read_excel(data_path+'pumpkin.xlsx')
