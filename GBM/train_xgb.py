...
from sklearn.metrics import mean_squared_error, accuracy_score as get_acc
import os
import pickle
...
def train(dtrain, dtest, y_test, params, output_path, callback, arg, num_rounds, model):
    ...
    for i in range(1,num_rounds+1):
        output.append([])
        ans = model.predict(dtest,ntree_limit=i)
        for xxx in range(len(ans)) :
            ans[xxx] = np.log(ans[xxx]/(1-ans[xxx]))
        for j in range(len(ans)):
            output[i].append(ans[j]-tmp_ans[j])
        tmp_ans=ans
...
def xgbTrain(X, y, kfold, params, callback, arg, arg_param, path, num_round, statistic_cb, info):
    """
    :param X: 输入数据部分
    :param y: label部分
    :param kfold: fold数
    :param params: xgboost参数表
    :param callback: 回调函数
    :param arg: 回调函数需要的参数
    ...
    for train_index, test_index in kf.split(X,y):
        ...
        for i in range(1, num_round + 1):
            traindata_output.append([])
            ans = model.predict(dtrain, ntree_limit=i, output_margin=True)
            # for xxx in range(len(ans)) :
            #     ans[xxx] = np.log(ans[xxx]/(1-ans[xxx]))
            for j in range(len(ans)):
